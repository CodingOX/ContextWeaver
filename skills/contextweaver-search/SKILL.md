---
name: contextweaver-search
description: Use when you need to search a codebase semantically — find code by describing what it does, explore how a feature is implemented, locate relevant source files for a task, or understand the architecture of a project. Also use when ContextWeaver isn't working (env not configured, index missing) and you need to diagnose or set it up. This skill should be used for ANY non-trivial code exploration task in a ContextWeaver-indexed repository, even if the user doesn't explicitly mention "search" or "ContextWeaver".
---

# ContextWeaver 语义搜索指南

## 概述

ContextWeaver 是语义检索引擎，采用混合搜索（向量 + 词法 + RRF 融合）。通过 CLI 命令 `contextweaver search` 使用。
...
然后让用户编辑 `~/.contextweaver/.env`，至少填写：
...
| 忘记先检查环境配置 | 搜索前先 `cat ~/.contextweaver/.env` |
| 未索引就直接搜索 | 先执行 `contextweaver index` |
...
# 2. 编辑配置文件填写 API Key
vim ~/.contextweaver/.env

# 3. 对项目建立索引（在项目目录下执行）
contextweaver index

# 4. 搜索
contextweaver search --information-request "How does X work"
```
